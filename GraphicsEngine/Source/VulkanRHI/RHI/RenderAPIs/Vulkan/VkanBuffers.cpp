...
uint32_t findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties)
{
	...
	vkGetPhysicalDeviceMemoryProperties(VKanRHI::RHIinstance->physicalDevice, &memProperties);
...
void VKanBuffer::CreateVertexBuffer(int Stride, int ByteSize, EBufferAccessType::Type Accesstype/* = EBufferAccessType::Static*/)
{
	...
	if (vkCreateBuffer(VKanRHI::RHIinstance->device, &bufferInfo, nullptr, &vertexbuffer) != VK_SUCCESS)
	{
		throw std::runtime_error("failed to create vertex buffer!");
	}
	VkMemoryRequirements memRequirements;
	vkGetBufferMemoryRequirements(VKanRHI::RHIinstance->device, vertexbuffer, &memRequirements);
	VkMemoryAllocateInfo allocInfo = {};
	allocInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
	allocInfo.allocationSize = memRequirements.size;
	allocInfo.memoryTypeIndex = findMemoryType(memRequirements.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);

	if (vkAllocateMemory(VKanRHI::RHIinstance->device, &allocInfo, nullptr, &vertexBufferMemory) != VK_SUCCESS)
	{
		throw std::runtime_error("failed to allocate vertex buffer memory!");
	}
	vkBindBufferMemory(VKanRHI::RHIinstance->device, vertexbuffer, vertexBufferMemory, 0);
}

void VKanBuffer::UpdateVertexBuffer(void * data, size_t length)
{
	void* GPUdata;
	vkMapMemory(VKanRHI::RHIinstance->device, vertexBufferMemory, 0, TotalByteSize, 0, &GPUdata);
	memcpy(GPUdata, data, length);
	vkUnmapMemory(VKanRHI::RHIinstance->device, vertexBufferMemory);
}
...
void createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory)
{
	...
	if (vkCreateBuffer(VKanRHI::RHIinstance->device, &bufferInfo, nullptr, &buffer) != VK_SUCCESS)
	{
		throw std::runtime_error("failed to create buffer!");
	}

	VkMemoryRequirements memRequirements;
	vkGetBufferMemoryRequirements(VKanRHI::RHIinstance->device, buffer, &memRequirements);
	...
	if (vkAllocateMemory(VKanRHI::RHIinstance->device, &allocInfo, nullptr, &bufferMemory) != VK_SUCCESS)
	{
		throw std::runtime_error("failed to allocate buffer memory!");
	}

	vkBindBufferMemory(VKanRHI::RHIinstance->device, buffer, bufferMemory, 0);
}
