...
float4 main(VS_OUTPUT input) : SV_Target
{
	...
	[branch]
	if (!ShouldShadePixel_PS(input.uv,Resolution,input.uv))
	{
	...
	VRRTemp[Pos.xy + int2(1, 0)] = float4(output, 1.0f);
	VRRTemp[Pos.xy + int2(0, -1)] = float4(output, 1.0f);
	VRRTemp[Pos.xy + int2(1, -1)] = float4(output, 1.0f);
#endif
